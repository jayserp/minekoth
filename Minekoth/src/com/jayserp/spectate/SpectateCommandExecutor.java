...
import com.jayserp.minekoth.Minekoth;
...
public class SpectateCommandExecutor implements CommandExecutor {
	...
	public SpectateAPI spectateApi;

	public SpectateCommandExecutor(Minekoth plugin) {
		this.plugin = plugin;
		spectateApi = new SpectateAPI(plugin);
	}
			...
			if (args.length > 0) {
				if (args[0].equalsIgnoreCase("off")) {
					if (isSpectating.get(cmdsender.getName()) != null) {
						if (isSpectating.get(cmdsender.getName())) {
							...
							cmdsender.sendMessage("You have stopped spectating " + target.get(cmdsender.getName()) + ".");
					...
					cmdsender.sendMessage("You are not currently spectating anyone.");
					return true;
				}

				if (args[0].equalsIgnoreCase("mode")) {
					if (args.length < 2) {
						cmdsender.sendMessage("Error: You must enter the mode type.");
						return true;
					}
					...
					if (args[1].equalsIgnoreCase("1") || args[1].equalsIgnoreCase("default")) {
						if (mode.get(cmdsender.getName()) == null || mode.get(cmdsender.getName()).equals("1")) {
							cmdsender.sendMessage("You are already in this mode.");
							return true;
						}
						mode.put(cmdsender.getName(), "1");
						cmdsender.sendMessage("You are now using the default spectate mode.");
						return true;
					}
					...
					if (args[1].equalsIgnoreCase("2") || args[1].equalsIgnoreCase("scroll")) {
						if (mode.get(cmdsender.getName()) == null) {
							...
							cmdsender.sendMessage("You are now using the scroll spectate mode.");
							return true;
						}
				...
				if (args[0].equalsIgnoreCase("scan")) {
					if (isScanning.get(cmdsender.getName()) != null) {
						if (isScanning.get(cmdsender.getName())) {
							cmdsender.sendMessage("Error: You are already scanning.");
							return true;
						}
					}
					...
					if (args.length < 2) {
						cmdsender.sendMessage("Error: You must enter an interval.");
						return true;
					}
					...
					try {
						...
						cmdsender.sendMessage("Error: " + args[1] + " is not a number.");
						return true;
					}

					if (interval <= 0) {
						cmdsender.sendMessage("Error: Interval must be greater than 0.");
						return true;
					}
					...
					try {
						...
						cmdsender.sendMessage("Error: There is nobody to spectate.");
						return true;
					}
				...
				if (targetPlayer != null) {
					if (cmdsender.getName() == targetPlayer.getName()) {
						cmdsender.sendMessage("Did you really just try to spectate yourself?");
						return true;
					}
					...
					if (isSpectating.get(cmdsender.getName()) != null) {
						if (isSpectating.get(cmdsender.getName())) {
							if (targetPlayer.getName().equals(target.get(cmdsender.getName()))) {
								cmdsender.sendMessage("You are already spectating them.");
								return true;
							}
						}
					}
					...
					if (isSpectating.get(targetPlayer.getName()) != null) {
						if (isSpectating.get(targetPlayer.getName())) {
							cmdsender.sendMessage("They are currently spectating someone.");
							return true;
						}
					}

					if (targetPlayer.isDead()) {
						cmdsender.sendMessage("They are currently dead.");
						return true;
					}

					if (plugin.conf.getBoolean("canspectate Permission Enabled?") == true) {
						if (targetPlayer.hasPermission("spectate.cantspectate")) {
							cmdsender.sendMessage("They can not be spectated.");
							return true;
						}
					}
				...
				if (args[0].equalsIgnoreCase("herobrine")) {
					cmdsender.sendMessage("You can't watch Herobrine, only he can watch you ;)");
					return true;
				}
				cmdsender.sendMessage("Error: Player is not online");
				return true;
			}
			...
			if (plugin.CommandExecutor.mode.get(cmdsender.getName()) != null) {
				if (plugin.CommandExecutor.mode.get(cmdsender.getName()).equals("2")) {
					...
					try {
							...
							if (isSpectating.get(cmdsender.getName())) {
								if (target.get(cmdsender.getName()) != null) {
									if (players[0].getName().equals(target.get(cmdsender.getName()))) {
										cmdsender.sendMessage("Error: You are already spectating this player.");
										return true;
									}
								}
							}
						...
						cmdsender.sendMessage("There is nobody to spectate.");
						return true;
					}
				}
			}
			cmdsender.sendMessage("Error: No player target");
			return true;
		}
		return true;
	}
